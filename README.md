...
源码仓库应提交源码、示例项目、工具脚本和 lockfile；不要提交 `node_modules/`、`dist/`、`release/` 或本地配置文件。正式 release 建议在 GitHub Releases 中上传打包后的便携 EXE 或压缩包。
...
本仓库使用 `rollup` 的 WASM 包作为开发依赖，以避开部分 Windows、Node 24、非 ASCII 路径组合下 native Rollup 包加载失败的问题。
